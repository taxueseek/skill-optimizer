<p align="center">
  <img src="./assets/readme/hero.svg" width="100%" alt="Skill Optimizer：写完还能证明它好用。三条能跑的流水线。">
</p>

# Skill Optimizer

写一个 Skill 容易，写完还能证明它好用很难。这不是一份通用指南，是三条能直接跑的流水线：Grok Build、Kimi Code、MiMo Code 各一条。打包前共用同一套验证门。

对照 Claude Code 自带的 `skill-creator`，这三个版本按各平台原生工具写，不拿一层适配去抹平差异。

**v1.1.1** 加上共享验证门：结构站不站得住、触发描述是不是只做门控、资产能不能搬走、有没有可重复的自动检查。学的是可迁移的工程习惯，不是某次项目的事故清单。
